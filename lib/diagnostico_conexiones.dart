...
class DiagnosticoScreen extends StatelessWidget {
  ...
  // ── KPI cards (Optimized for same size and responsive text) ─────────────────
  Widget _buildKpis() => LayoutBuilder(
    builder: (context, constraints) {
      // Si la pantalla es muy estrecha (móvil pequeño), usamos 2 filas
      if (constraints.maxWidth < 500) {
        return Column(
          children: [
            Row(children: [
              Expanded(child: _KpiCard(label: 'P. enviados', value: '12,540', sub: 'total', valueColor: kText)),
              const SizedBox(width: 12),
              Expanded(child: _KpiCard(label: 'P. recibidos', value: '12,148', sub: 'total', valueColor: kText)),
            ]),
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: _KpiCard(label: 'P. perdidos', value: '392', sub: '(3.12%)', valueColor: kRed)),
              const SizedBox(width: 12),
              Expanded(child: _KpiCard(label: 'Reconexiones', value: '8', sub: 'hoy', valueColor: kGreen)),
            ]),
          ],
        );
      }
      // Para tabletas o móviles más anchos, 4 en una fila
      return Row(children: [
        Expanded(child: _KpiCard(label: 'Paquetes enviados', value: '12,540', sub: 'total', valueColor: kText)),
        const SizedBox(width: 10),
        Expanded(child: _KpiCard(label: 'Paquetes recibidos', value: '12,148', sub: 'total', valueColor: kText)),
        const SizedBox(width: 10),
        Expanded(child: _KpiCard(label: 'Paquetes perdidos', value: '392', sub: '(3.12%)', valueColor: kRed)),
        const SizedBox(width: 10),
        Expanded(child: _KpiCard(label: 'Reconexiones', value: '8', sub: 'hoy', valueColor: kGreen)),
      ]);
    }
...
// ─── Helpers (Modified for same size and auto-adjusting content) ──────────────
class _KpiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    height: 125, // Altura fija para que todos sean iguales
    ...
    child: Column(
      ...
      children: [
        ...
        FittedBox(
          ...
          child: Text(label, style: const TextStyle(fontSize: 11, color: kMuted2, fontWeight: FontWeight.w600)),
        ),
        // Valor numérico (se ajusta al ancho del cuadro)
        Expanded(
          child: Center(
            child: FittedBox(
              fit: BoxFit.scaleDown,
        ...
        // Sub-etiqueta inferior
        FittedBox(
          fit: BoxFit.scaleDown,
          child: Text(sub, style: const TextStyle(fontSize: 11, color: kMuted)),
        ),
      ],
    ),
  );
