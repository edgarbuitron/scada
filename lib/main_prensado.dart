...
class _ScadaPrensadoState extends State<ScadaPrensadoScreen> {
  ...
  void _log(String msg, _PLogType type) {
    ...
    if (mounted) {
      ...
      switch (type) {
        case _PLogType.error: logTypeForHistory = 'Critico'; break;
        case _PLogType.audit: logTypeForHistory = 'Advertencia'; break;
  ...
  Widget _dtRect(String label, bool active, Color color,
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: active ? color.withValues(alpha: 0.22) : const Color(0xFF1A2233),
        border: Border.all(
  ...
  Widget _sensorDot(String id, bool active) => AnimatedContainer(
    ...
    decoration: BoxDecoration(
      ...
      color: active ? _kGreen.withValues(alpha: 0.2) : Colors.transparent,
      border: Border.all(
          color: active ? _kGreen : const Color(0xFF334455), width: 2),
      boxShadow: active
          ? [BoxShadow(color: _kGreen.withValues(alpha: 0.6), blurRadius: 10)]
          : null,
    ),
    alignment: Alignment.center,
    child: Text(id,
    ...
    child: Row(mainAxisSize: MainAxisSize.min, children: [
      AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: active
              ? [BoxShadow(color: _kMachine.withValues(alpha: 0.7), blurRadius: 6)]
              : null,
        ),
      ),
  ...
  Widget _sensorRow(_PSensorModel s) => GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color: s.active ? _kGreen.withValues(alpha: 0.4) : _kBorder),
        borderRadius: BorderRadius.circular(4),
      ),
      child: Row(children: [
        Expanded(
        ...
        AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: s.active
                ? [
              BoxShadow(
                  color: _kGreen.withValues(alpha: 0.7), blurRadius: 8)
            ]
                : null,
          ),
        ),
  ...
  Widget _actuatorRow(_PActuatorModel a) => Container(
    ...
    decoration: BoxDecoration(
      color:
      a.on ? _kMachine.withValues(alpha: 0.08) : Colors.black.withValues(alpha: 0.3),
      border:
      Border.all(color: a.on ? _kMachine.withValues(alpha: 0.5) : _kBorder),
      borderRadius: BorderRadius.circular(4),
    ),
    child: Row(children: [
      Expanded(
